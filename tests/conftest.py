...
def migrated_db(tmp_path_factory) -> str:
    ...
    ⚠️ لكن الجداول المشتقّة تُكتب: forecasts وrecommendations تتراكم عبر
    الجلسة ولا ينظّفها أحد. تنجو الاختبارات القائمة لأنها لا تؤكّد إلا على
    ما كتبته للتوّ — لا على مجموع ولا على جدول فارغ. نجاةٌ بالصدفة لا
    بالتصميم: أول اختبار يقيس حالة عامة يفشل بسبب صفٍّ كتبه اختبار آخر.

    من يحتاج بداية نظيفة يعزل قاعدته الخاصة بدل هذه المشتركة — انظر نمط
    isolated_db في tests/test_stock_upload_ui.py.
    """
