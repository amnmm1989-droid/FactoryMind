...
class RiskScore:
    ...
    @property
    def factor_counts(self) -> tuple[int, int]:
        """(المحسوبة، الإجمالي) — لتُعرَض ككسر «4/5» لا كنسبة مئوية.

        بيانات لا نص: الواجهة تصوغها، والكيان يعدّ. الإجمالي مشتقّ لا
        مثبَّت، فإضافة عامل سادس لا تترك «/5» كذبةً في شاشتين.
        """
        known = len(self.known_factors)
        return known, known + len(self.missing_factors)

    @property
    def confidence(self) -> float:
        """نسبة العوامل المعروفة (0-1) — مقياس صلابة **درجة الخطورة**.

        ⚠️ ليست ثقةً في التنبؤ، ولا احتمالاً. تقول: «كم عاملاً من الخمسة
        دخل في حساب هذه الدرجة؟» فحسب.

        ولهذا لا تُعرَض في الواجهة كنسبة مئوية: «80%» بجانب عمود دقّة
        WAPE تُقرأ حتماً «التنبؤ موثوق 80%» — وهو ليس ما تقوله. تُعرَض
        «4/5» بدلاً منها، وهي صيغة لا تحتمل ذلك اللبس.
        """
...
class ProductionRecommendation:
    ...
    @property
    def units_at_risk(self) -> float:
        """الكمية الموصى بها مضروبة في درجة الخطورة — بوحدات الإنتاج.

        ⚠️ يُصلح عطلاً قِيس على الملفات الخمسة: الترتيب بالخطورة وحدها
        يُصعّد أكياس 60g المتذبذبة ويُغرق أكبر منتجات المصنع. في الملف
        السنوي كانت الصفوف الخمسون المعروضة تغطّي **6%** من الإنتاج،
        وأكبر منتج (86,967 وحدة) يقع في المرتبة 91 — خارج الشاشة تماماً.

        الخطورة نسبة بلا مقياس: 70% على 6 وحدات و70% على 86,967 وحدة
        درجتهما واحدة، وأثرهما ليس واحداً. الضرب يُعيد المقياس، فيصير
        الرقم قابلاً للقراءة كما هو: «كم وحدة من هذه التوصية معرَّضة
        لعدم اليقين؟» — لا مؤشراً مركَّباً بلا وحدة.

        درجة مفقودة (risk=None) تُعامَل صفراً هنا لا واحداً: منتج لم
        تُحسب خطورته لا يُدفع إلى رأس القائمة بافتراض متشائم.
        """
        if self.risk is None:
            return 0.0
        return self.recommended_quantity * (self.risk.score / 100.0)
