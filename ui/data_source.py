...
def products_by_volume(products: dict[str, list[float]]) -> list[str]:
    """أسماء المنتجات مرتّبةً بالحجم التاريخي تنازلياً — لقوائم الاختيار.

    ⚠️ يُصلح عطلاً قِيس على الملفات الخمسة. كانت الصفحات ذات المنتج الواحد
    (Forecasting، Product Intelligence، Advanced Analytics) تستخدم
    `sorted(products)`، فتفتح كلّها على أول اسم أبجدياً. على هذا الكتالوج
    ذلك المنتج هو `Brazil - Morada () (60g)`:

    | الملف | حجمه | حالة الصفحة عند أول فتح |
    |---|---|---|
    | يومي | 0.03% (#86) | يعمل |
    | أسبوعي | 0.02% (#88) | يعمل |
    | شهري | 0.03% (#86) | تحذير "لم يُقيَّم أي نموذج" (0/5) |
    | ربعي | 0.03% (#86) | تحذير (0/2) |
    | سنوي | **0%** (منتج ميت) | **خطأ أحمر / صفحة فارغة** |

    أي أن الأداة تفتح على 0.03% من إنتاج المصنع في أحسن الأحوال، وعلى
    شاشة تبدو معطوبة في ثلاثة ملفات من خمسة — بينما أكبر منتج (15.8% من
    الإنتاج) لا يراه أحد ما لم يبحث عنه بالاسم.

    الترتيب بالحجم يُصلح الافتراضي **وترتيب التصفّح** بحركة واحدة، ولا
    يُصعّب إيجاد منتج بعينه: قوائم Streamlit تقبل الكتابة للبحث.

    التعادل يُفَضّ أبجدياً كي يبقى الترتيب ثابتاً بين التشغيلات — قائمة
    تتبدّل عشوائياً تُربك من يعود إليها.
    """
    return sorted(products, key=lambda name: (-sum(products[name]), name))
